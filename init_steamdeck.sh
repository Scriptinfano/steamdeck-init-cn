...
echo "======================================================="
echo "请确保您当前在包含 PluginLoader 和 tomoon 文件夹的目录下执行此脚本。"
...
# 0. 网络检查与 IP 获取
echo -e "\n[0/10] 检查网络连接状态..."
DECK_IP=$(ip -4 a show wlan0 2>/dev/null | grep -oP '(?<=inet\s)\d+(\.\d+){3}')

if [ -z "$DECK_IP" ]; then
    echo "=========================================================================="
    echo "❌ 错误：未检测到 WiFi 连接！"
    echo "本脚本的后续步骤（如安装商店源、下载插件、配置 SSH 等）需要网络支持。"
    echo "请先在右上角连接 WiFi（确保与您的 MacBook 在同一局域网），然后重新运行脚本。"
    echo "=========================================================================="
    exit 1
else
    echo "网络已连接。"
    echo "-------------------------------------------------------"
    echo "🌐 您的 Steam Deck 局域网 IP 地址为: $DECK_IP"
    echo "💡 请记住这个 IP，稍后您可以在 MacBook 上使用 connect_deck.sh 远程连接"
    echo "-------------------------------------------------------"
fi
